...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    if not device.is_dali:
        return

    entities = []
    for key, state in device.dali_channels.items():
        entities.append(DaliBrightnessSensor(hass, entry, device, key, state))
        if state.get("kind") == "cct":
            entities.append(DaliColorTempSensor(hass, entry, device, key, state))
    ...
    # DALI channels can also be discovered AFTER platform setup, since
    # they arrive via live *AR=/*AH= pushes rather than an upfront query
    # (see dali.py). Watch for new ones and add entities on the fly.
    @callback
    def _on_new_state(event):
        ...
        try:
            ...
            pass
        if key in device.dali_channels and not any(
            getattr(e, "_dali_key", None) == key for e in entities
        ):
